...
#include "string.h"
#include "brd_cfg.h"
#include "led.h"	//去掉这句就会报错说SPI4没有定义，奇怪
...
//    M8266HostIf_SPI_SetSpeed(SPI_BaudRatePrescaler_2);					// Setup SPI Clock. Here 54/2  = 27MHz for STM32F7xx
		M8266HostIf_SPI_SetSpeed(SPI_BaudRatePrescaler_4);					// Set SCLK = 54/4 = 13.5MHz for STM32F7xx	------ Ben
	#endif
//		 spi_clk = 27000000;	//Default
		spi_clk = 13500000;		// ------ Ben
...
/****************************************************这里设置AP参数****************************************************/
#if 1 // If you expect to change the ap info overrding the default loaded from flash on bootup, enable it by "#if 1"
{
// u8 M8266WIFI_SPI_Config_AP(u8 ssid[13+1], u8 password[13+1], u8 enc, u8 channel, u8 saved, u16* status);	
	if(M8266WIFI_SPI_Config_AP("ELONXI-US", "123456789", 4, 6, 0, &status)==0)  // set to 4=WPA_WPA2_PSK, not saved // 0=OPEN, 1=WEP, 2=WPA_PSK, 3=WPA2_PSK, 4=WPA_WPA2_PSK
    return 0;
}
#endif
...
//u8 M8266WIFI_SPI_Get_Opmode(u8* op_mode, u16* status);	
	   ...
	   ||(sta_ap_mode == 3))  // if STA+AP mode
	{
		...
		//u8 M8266WIFI_SPI_STA_Connect_Ap(u8 ssid[32], u8 password[64], u8 saved, u8 timeout_in_s, u16* status);
 		     if(M8266WIFI_SPI_STA_Connect_Ap("Router", "123456789", 0, 20, &status) == 0) // not saved, timeout=20s
				 return 0;			 			 
#endif
