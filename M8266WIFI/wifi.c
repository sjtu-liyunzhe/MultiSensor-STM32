...
#else
#define TEST_REMOTE_ADDR   		"192.168.4.1"   // "www.baidu.com"   wifi模块地址
#define TEST_REMOTE_PORT  	    4321			// wifi模块的密码
#endif		
...
{
	#define SEND_SIZE	sizeof(FIFO_buffer)
	u16 sent = 0;
	link_no = 0;	//Send to connection with link_no = 0
	
	//Use simple but less-efficiency transmission
	//u32 M8266WIFI_SPI_Send_Data_Block(u8 Data[], u32 Data_len, u16 tcp_packet_size, u8 link_no, u16* status); 
	
	sent = M8266WIFI_SPI_Send_Data_Block(FIFO_buffer, SEND_SIZE, 1460, link_no, &status);
  
	if( (sent!=SEND_SIZE) || ( (status&0xFF)!= 0 ) ) 
	{
...
void Wifi_Test(void)
{
	...
	{
	#define TEST_SEND_DATA_SIZE   1000 //2048 // 1472 //1460
...
//	#endif	
	...
	#if 1  // if use simple but less-efficiency transmission, changed to "#if 1"
				//u32 M8266WIFI_SPI_Send_Data_Block(u8 Data[], u32 Data_len, u16 tcp_packet_size, u8 link_no, u16* status); 
...
//	#else // complex but high-efficiency transsmission test	（这个函数丢帧很厉害，不用！！！）
