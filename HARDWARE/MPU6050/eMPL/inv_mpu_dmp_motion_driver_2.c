...
{
    return mpu_load_firmware_2(DMP_CODE_SIZE, dmp_memory, sStartAddress,
        DMP_SAMPLE_RATE);
}
...
int dmp_set_orientation_2(unsigned short orient)
{
    ...
    if (mpu_write_mem_2(FCFG_1, 3, gyro_regs))
        return -1;
    if (mpu_write_mem_2(FCFG_2, 3, accel_regs))
        return -1;
    ...
    if (mpu_write_mem_2(FCFG_3, 3, gyro_regs))
        return -1;
    if (mpu_write_mem_2(FCFG_7, 3, accel_regs))
        return -1;
    dmp.orient = orient;
    return 0;
}
...
int dmp_set_gyro_bias_2(long *bias)
{
    ...
    if (mpu_write_mem_2(D_EXT_GYRO_BIAS_X, 4, regs))
        return -1;
    ...
    if (mpu_write_mem_2(D_EXT_GYRO_BIAS_Y, 4, regs))
        return -1;
    ...
    return mpu_write_mem_2(D_EXT_GYRO_BIAS_Z, 4, regs);
}
...
int dmp_set_accel_bias_2(long *bias)
{
    ...
    return mpu_write_mem_2(D_ACCEL_BIAS, 12, regs);
}
...
int dmp_set_fifo_rate_2(unsigned short rate)
{
    ...
    if (mpu_write_mem_2(D_0_22, 2, tmp))
        return -1;
    if (mpu_write_mem_2(CFG_6, 12, (unsigned char*)regs_end))
        return -1;
...
int dmp_set_tap_thresh_2(unsigned char axis, unsigned short thresh)
{
    ...
    if (axis & TAP_X) {
        if (mpu_write_mem_2(DMP_TAP_THX, 2, tmp))
            return -1;
        if (mpu_write_mem_2(D_1_36, 2, tmp+2))
            return -1;
    }
    if (axis & TAP_Y) {
        if (mpu_write_mem_2(DMP_TAP_THY, 2, tmp))
            return -1;
        if (mpu_write_mem_2(D_1_40, 2, tmp+2))
            return -1;
    }
    if (axis & TAP_Z) {
        if (mpu_write_mem_2(DMP_TAP_THZ, 2, tmp))
            return -1;
        if (mpu_write_mem_2(D_1_44, 2, tmp+2))
            return -1;
    }
    return 0;
}
...
int dmp_set_tap_axes_2(unsigned char axis)
{
    ...
    if (axis & TAP_Z)
        tmp |= 0x03;
    return mpu_write_mem_2(D_1_72, 1, &tmp);
}
...
int dmp_set_tap_count_2(unsigned char min_taps)
{
    ...
    tmp = min_taps - 1;
    return mpu_write_mem_2(D_1_79, 1, &tmp);
}
...
int dmp_set_tap_time_2(unsigned short time)
{
    ...
    return mpu_write_mem_2(DMP_TAPW_MIN, 2, tmp);
}
...
int dmp_set_tap_time_multi_2(unsigned short time)
{
    ...
    tmp[0] = (unsigned char)(dmp_time >> 8);
    tmp[1] = (unsigned char)(dmp_time & 0xFF);
    return mpu_write_mem_2(D_1_218, 2, tmp);
}
...
int dmp_set_shake_reject_thresh_2(long sf, unsigned short thresh)
{
    ...
    return mpu_write_mem_2(D_1_92, 4, tmp);
}
...
int dmp_set_shake_reject_time_2(unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem_2(D_1_90,2,tmp);
}
...
int dmp_set_shake_reject_timeout_2(unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem_2(D_1_88,2,tmp);
}
...
int dmp_get_pedometer_step_count_2(unsigned long *count)
{
    ...
    if (mpu_read_mem_2(D_PEDSTD_STEPCTR, 4, tmp))
        return -1;
...
int dmp_set_pedometer_step_count_2(unsigned long count)
{
    ...
    return mpu_write_mem_2(D_PEDSTD_STEPCTR, 4, tmp);
}
...
int dmp_get_pedometer_walk_time_2(unsigned long *time)
{
    ...
    if (mpu_read_mem_2(D_PEDSTD_TIMECTR, 4, tmp))
        return -1;
...
int dmp_set_pedometer_walk_time_2(unsigned long time)
{
    ...
    return mpu_write_mem_2(D_PEDSTD_TIMECTR, 4, tmp);
}
...
int dmp_enable_feature_2(unsigned short mask)
{
    ...
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ANDROID_ORIENT))
        ...
        tmp[0] = 0xD8;
    mpu_write_mem_2(CFG_27,1,tmp);

    if (mask & DMP_FEATURE_GYRO_CAL)
        dmp_enable_gyro_cal_2(1);
    else
        dmp_enable_gyro_cal_2(0);
    ...
    if (mask & DMP_FEATURE_SEND_ANY_GYRO) {
        ...
        mpu_write_mem_2(CFG_GYRO_RAW_DATA, 4, tmp);
    }

    if (mask & DMP_FEATURE_TAP) {
        /* Enable tap. */
        tmp[0] = 0xF8;
        mpu_write_mem_2(CFG_20, 1, tmp);
        dmp_set_tap_thresh_2(TAP_XYZ, 250);
        dmp_set_tap_axes_2(TAP_XYZ);
        dmp_set_tap_count_2(1);
        dmp_set_tap_time_2(100);
        dmp_set_tap_time_multi_2(500);

        dmp_set_shake_reject_thresh_2(GYRO_SF, 200);
        dmp_set_shake_reject_time_2(40);
        dmp_set_shake_reject_timeout_2(10);
    } else {
        tmp[0] = 0xD8;
        mpu_write_mem_2(CFG_20, 1, tmp);
    }
    ...
    if (mask & DMP_FEATURE_ANDROID_ORIENT) {
        ...
        tmp[0] = 0xD8;
    mpu_write_mem_2(CFG_ANDROID_ORIENT_INT, 1, tmp);

    if (mask & DMP_FEATURE_LP_QUAT)
        dmp_enable_lp_quat_2(1);
    else
        dmp_enable_lp_quat_2(0);

    if (mask & DMP_FEATURE_6X_LP_QUAT)
        dmp_enable_6x_lp_quat_2(1);
    else
        dmp_enable_6x_lp_quat_2(0);
...
int dmp_enable_gyro_cal_2(unsigned char enable)
{
    if (enable) {
        ...
        return mpu_write_mem_2(CFG_MOTION_BIAS, 9, regs);
    } else {
...
int dmp_enable_lp_quat_2(unsigned char enable)
{
    ...
    mpu_write_mem_2(CFG_LP_QUAT, 4, regs);

    return mpu_reset_fifo_2();
}
...
int dmp_set_interrupt_mode_2(unsigned char mode)
{
    ...
    case DMP_INT_CONTINUOUS:
        return mpu_write_mem_2(CFG_FIFO_ON_EVENT, 11,
    ...
    unsigned long *timestamp, short *sensors, unsigned char *more)
{
    ...
    /* Get a packet. */
    if (mpu_read_fifo_2_stream(dmp.packet_length, fifo_data, more))
        return -1;
